<x-app-layout title="Reading Plans" :breadcrumbs="$breadcrumbs" main-class="px-4 py-5 sm:px-6 lg:px-7">
    ...
    <div class="space-y-5">
        ...
        <form method="GET" action="{{ route('bible.plans') }}" class="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-5">
            <label class="relative"><span class="sr-only">Search plans</span><i data-lucide="search" class="absolute left-3 top-3 size-4 text-slate-400"></i><input name="q" value="{{ $filters['q'] }}" class="h-10 w-full rounded-xl border-slate-200 pl-9 pr-3 text-sm" placeholder="Search reading plans..."></label>
            <select name="category" class="h-10 rounded-xl border-slate-200 px-3 text-sm"><option value="">All Categories</option>@foreach($categories as $category)<option value="{{ $category->category }}" @selected($filters['category'] === $category->category)>{{ $category->category }}</option>@endforeach</select>
            <select name="status" class="h-10 rounded-xl border-slate-200 px-3 text-sm"><option value="">All Statuses</option><option value="active" @selected($filters['status'] === 'active')>Active</option><option value="completed" @selected($filters['status'] === 'completed')>Completed</option></select>
            <select name="duration" class="h-10 rounded-xl border-slate-200 px-3 text-sm"><option value="">Any Duration</option><option value="30" @selected($filters['duration'] === '30')>30 days or less</option><option value="90" @selected($filters['duration'] === '90')>90 days or less</option><option value="365" @selected($filters['duration'] === '365')>Up to one year</option></select>
