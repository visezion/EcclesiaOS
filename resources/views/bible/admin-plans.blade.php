<x-app-layout title="Manage Reading Plans" :breadcrumbs="$breadcrumbs" main-class="px-4 py-5 sm:px-6 lg:px-7">
    <div x-data="{ createOpen: {{ $errors->any() ? 'true' : 'false' }}, editing: null }" class="space-y-5">
            ...
            @forelse($plans as $plan)
                ...
                <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div x-cloak x-show="editing === {{ $plan->id }}" x-transition class="border-t border-slate-200 bg-slate-50/60 p-5">
                        <form method="POST" action="{{ route('bible.admin.plans.update', $plan) }}" class="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2">@csrf @method('PUT')
                            <label class="text-sm font-bold text-slate-700">Plan name<input name="name" value="{{ $plan->name }}" required maxlength="160" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 focus:border-violet-500 focus:ring-violet-200"></label>
                            <label class="text-sm font-bold text-slate-700">Category<input name="category" value="{{ $plan->category }}" required maxlength="80" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 focus:border-violet-500 focus:ring-violet-200"></label>
                            <label class="text-sm font-bold text-slate-700 sm:col-span-2">Description<textarea name="description" required maxlength="2000" rows="3" class="mt-1.5 w-full rounded-xl border border-slate-300 bg-white p-3 focus:border-violet-500 focus:ring-violet-200">{{ $plan->description }}</textarea></label>
                            <label class="text-sm font-bold text-slate-700 sm:col-span-2">Daily schedule <span class="font-normal text-slate-400">(one day per line)</span><textarea name="schedule" required rows="12" class="mt-1.5 w-full rounded-xl border border-slate-300 bg-white p-3 font-mono text-xs leading-6 focus:border-violet-500 focus:ring-violet-200">{{ $schedule }}</textarea><span class="mt-1 block text-xs font-normal text-slate-500">Format: Day title | Bible passages | Optional reflection</span></label>
                            <label class="flex items-center gap-2 text-sm font-bold text-slate-700"><input name="is_recommended" value="1" type="checkbox" @checked($plan->is_recommended) class="rounded border border-violet-300 text-violet-600 focus:ring-violet-500">Recommended for members</label>
        ...
        <div x-cloak x-show="createOpen" x-transition.opacity class="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/45 p-4" @keydown.escape.window="createOpen = false" @click.self="createOpen = false">
            <form method="POST" action="{{ route('bible.admin.plans.store') }}" class="my-6 w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">@csrf
                ...
                <div class="mt-5 grid gap-4 sm:grid-cols-2">
                    <label class="text-sm font-bold text-slate-700">Plan name<input name="name" value="{{ old('name') }}" required maxlength="160" placeholder="30 Days in Psalms" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 focus:border-violet-500 focus:ring-violet-200"></label>
                    <label class="text-sm font-bold text-slate-700">Category<input name="category" value="{{ old('category') }}" required maxlength="80" placeholder="Psalms" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 focus:border-violet-500 focus:ring-violet-200"></label>
                    <label class="text-sm font-bold text-slate-700 sm:col-span-2">Description<textarea name="description" required maxlength="2000" rows="3" class="mt-1.5 w-full rounded-xl border border-slate-300 bg-white p-3 focus:border-violet-500 focus:ring-violet-200" placeholder="Explain what members will read and learn...">{{ old('description') }}</textarea></label>
                    <label class="text-sm font-bold text-slate-700 sm:col-span-2">Daily schedule <span class="font-normal text-slate-400">(one day per line)</span><textarea name="schedule" required rows="12" class="mt-1.5 w-full rounded-xl border border-slate-300 bg-white p-3 font-mono text-xs leading-6 focus:border-violet-500 focus:ring-violet-200" placeholder="Creation | Genesis 1-2 | Reflect on God as Creator&#10;The Fall | Genesis 3-4 | Consider the consequences of sin&#10;The Flood | Genesis 5-7">{{ old('schedule') }}</textarea><span class="mt-1 block text-xs font-normal text-slate-500">Format: Day title | Bible passages | Optional reflection. Use semicolons between separate passages.</span></label>
                    <label class="flex items-center gap-2 text-sm font-bold text-slate-700 sm:col-span-2"><input name="is_recommended" value="1" type="checkbox" @checked(old('is_recommended', true)) class="rounded border border-violet-300 text-violet-600 focus:ring-violet-500">Recommend this plan to members</label>
                </div>
