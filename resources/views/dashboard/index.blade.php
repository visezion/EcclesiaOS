<x-app-layout title="Dashboard">
        ...
        <x-dashboard-card title="Attendance Trend" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($summaryPanelCount, 4) }};">
            <div class="mb-4 flex items-start justify-between gap-3">
                <div>
                    ...
                    <div class="mt-1 text-sm text-slate-500">{{ $term('Average Attendance') }}</div>
                    <div class="mt-1 text-xs text-emerald-600"><i data-lucide="arrow-up" class="inline size-3"></i> {{ $attendanceTrend['change'] }} {{ $term('vs last 6 months') }}</div>
                </div>
                <select class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600">
                    <option>{{ $term('Last 6 Months') }}</option>
                </select>
            </div>
            <div class="h-60">
        ...
        <x-dashboard-card title="Giving Overview" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($summaryPanelCount, 3) }};">
            <div class="mb-4 flex items-start justify-between gap-3">
                <div>
                    ...
                    <div class="mt-1 text-sm text-slate-500">{{ $term('Total Giving') }}</div>
                    <div class="mt-1 text-xs text-emerald-600"><i data-lucide="arrow-up" class="inline size-3"></i> {{ $givingOverview['change'] }} {{ $term('vs last month') }}</div>
                </div>
                <select class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600">
                    <option>{{ $term('This Month') }}</option>
                </select>
            </div>
            <div class="h-60">
        ...
        <x-dashboard-card title="Book Store Snapshot" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($summaryPanelCount, 5) }};" :action="['label' => 'View Store', 'url' => route('bookstore.index')]">
            <div class="grid gap-3 sm:grid-cols-4">
                @foreach ($bookstore['totals'] as $total)
                    <div class="flex items-center gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                        ...
                        <div class="min-w-0">
                            <div class="truncate text-[11px] font-bold text-slate-500">{{ $term($total['label']) }}</div>
                            <div class="text-lg font-black">{{ $total['value'] }}</div>
                            <div class="truncate text-[11px] text-slate-500">{{ $term($total['note']) }}</div>
                        </div>
                    </div>
                @endforeach
            </div>
            <div class="mt-4 grid gap-4 lg:grid-cols-[1fr_230px]">
                <div class="overflow-x-auto">
                    <div class="mb-2 text-xs font-bold text-slate-700">{{ $term('Top Selling Books') }}</div>
                    <table class="table-compact">
                        <thead><tr><th>#</th><th>{{ $term('Book Title') }}</th><th>{{ $term('Sold') }}</th><th>{{ $term('Revenue') }}</th></tr></thead>
                        <tbody>
                ...
                <div>
                    <div class="mb-2 text-xs font-bold text-slate-700">{{ $term('Revenue by Category') }}</div>
    ...
    <div class="mt-4 grid gap-4 xl:grid-cols-12">
        ...
        <x-dashboard-card title="Asset Inventory Overview" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($operationsPanelCount, 4) }};" :action="['label' => 'View All Assets', 'url' => route('assets.index')]">
            <div class="overflow-x-auto">
                <table class="table-compact">
                    <thead><tr><th>{{ $term('Asset Category') }}</th><th>{{ $term('Total') }}</th><th>{{ $term('In Use') }}</th><th>{{ $term('Available') }}</th><th>{{ $term('Maintenance') }}</th><th>{{ $term('Status') }}</th></tr></thead>
                    <tbody>
                ...
                @foreach ($leadership as $row)
                    <div class="grid grid-cols-[1fr_auto_auto] items-center gap-3">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <div class="truncate text-sm font-bold text-slate-800">{{ $term($row['label']) }}</div>
                                <div class="text-xs text-slate-500">{{ $term($row['status']) }}</div>
                            </div>
                        </div>
        ...
        <x-dashboard-card title="Feedback System Overview" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($operationsPanelCount, 3) }};">
            <div class="grid grid-cols-3 gap-3 border-b border-slate-100 pb-4">
                <div><div class="text-xs font-bold text-slate-500">{{ $term('Survey Responses') }}</div><div class="mt-2 text-2xl font-black">{{ $feedback['summary']['responses'] }}</div></div>
                <div><div class="text-xs font-bold text-slate-500">{{ $term('Satisfaction') }}</div><div class="mt-2 flex items-center gap-1 text-2xl font-black text-slate-950"><i data-lucide="star" class="size-5 fill-amber-400 text-amber-400"></i> {{ $feedback['summary']['satisfaction'] }}</div></div>
                <div><div class="text-xs font-bold text-slate-500">NPS</div><div class="mt-2 text-2xl font-black">{{ $feedback['summary']['nps'] }}</div><div class="text-xs font-semibold text-emerald-600">{{ __(':count resolved', ['count' => $feedback['summary']['resolved']]) }}</div></div>
            </div>
            <div class="mt-4 grid gap-4 md:grid-cols-[1fr_150px]">
                <div class="space-y-2">
                    @foreach ($feedback['counts'] as $count)
                        <div class="flex justify-between text-sm"><span>{{ $term($count['label']) }}</span><span class="font-bold">{{ $count['value'] }}</span></div>
                    @endforeach
                </div>
            ...
            <div class="mt-4 rounded-lg bg-orange-50 p-3 text-sm font-semibold text-orange-700">{{ __('Follow-up Pending: :count', ['count' => $feedback['pending']]) }}</div>
        </x-dashboard-card>
        @endif
    ...
    <div class="mt-4 grid gap-4 xl:grid-cols-12">
        ...
        <x-dashboard-card title="Ministry & Department Performance" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($insightsPanelCount, 3) }};" :action="['label' => 'View All', 'url' => route('ministries.index')]">
            <table class="table-compact">
                <thead><tr><th>{{ $term($terminology['ministry_singular']) }}</th><th>{{ $term('Members') }}</th><th>{{ $term('Activities') }}</th><th>{{ $term('Impact') }}</th><th>{{ $term('Trend') }}</th></tr></thead>
                <tbody>
                ...
                <div class="space-y-3">
                    @foreach ($campuses as $campus)
                        <div class="flex justify-between gap-3 text-sm">
                            ...
                            <div class="text-right"><div class="font-bold">{{ $campus['attendance'] }}</div><div class="text-xs font-semibold text-emerald-600">{{ $term($campus['status']) }}</div></div>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <x-dashboard-card title="AI Insights & Smart Recommendations" class="dashboard-auto-span" style="--dashboard-span: {{ $dashboardSpan($insightsPanelCount, 3) }};" :action="['label' => 'View All Insights', 'url' => route('reports.index')]">
            <div class="mb-3 rounded-lg border border-dashed border-violet-200 bg-violet-50 p-3 text-xs text-violet-700">{{ $term('Insights are calculated from current database records.') }}</div>
            <div class="space-y-3">
                @foreach ($insights as $insight)
                    <div class="flex gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                        ...
                        <div class="min-w-0">
                            <div class="text-xs font-bold text-slate-500">{{ $term($insight['title']) }}</div>
                            <div class="truncate text-sm font-black text-slate-900">{{ $insight['value'] }}</div>
                            <div class="text-xs text-slate-500">{{ $term($insight['detail']) }}</div>
                        </div>
                    </div>
                @endforeach
            </div>
