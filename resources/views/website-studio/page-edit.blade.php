...
@endphp
<x-app-layout title="Edit Website Page: {{ $page->title }}" :breadcrumbs="$breadcrumbs">
    <div class="website-studio-admin mx-auto max-w-[1500px] space-y-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div class="mb-2 inline-flex items-center gap-2 rounded-full bg-violet-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.14em] text-violet-700"><i data-lucide="layers-3" class="size-3.5"></i>Website page editor</div><h1 class="text-3xl font-semibold tracking-tight text-slate-950">Edit Website Page</h1><p class="mt-1 text-sm text-slate-500">Update <span class="font-semibold text-slate-700">{{ $page->title }}</span>, organize its sections, and preview the finished page.</p></div><div class="flex gap-2"><a href="{{ route('website-studio.index') }}" class="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-600">All pages</a><a href="{{ route('website-studio.preview', $page) }}" target="_blank" class="inline-flex items-center gap-2 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-bold text-white hover:bg-violet-700"><i data-lucide="eye" class="size-4"></i>Preview</a></div></div>
        ...
        <form method="POST" action="{{ route('website-studio.pages.update', $page) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf @method('PUT')
            <input type="hidden" name="section_selection_configured" value="1">
    ...
    <style>
        .card-title{font-size:1.1rem;font-weight:800;color:#0f172a}.helper{margin-top:.25rem;font-size:.82rem;line-height:1.4;color:#64748b}.optional{font-weight:400;color:#94a3b8}.field-label{display:block;margin-bottom:.4rem;font-size:.72rem;font-weight:700;color:#475569}.field-input{display:block;width:100%;border-radius:.7rem;border:1px solid #e2e8f0;background:#fff;padding:.7rem .8rem;font-size:.875rem;color:#0f172a;outline:none}.field-input:focus{border-color:var(--studio-primary, var(--brand-primary, #8b5cf6));box-shadow:0 0 0 3px color-mix(in srgb, var(--studio-primary, var(--brand-primary, #8b5cf6)) 12%, transparent)}.color-input{display:block;height:43px;width:100%;border-radius:.7rem;border:1px solid #e2e8f0;background:#fff;padding:.25rem}.step-card{border:1px solid #e2e8f0;border-radius:1rem;background:#fff;padding:1rem}.step-card.active{border-color:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 38%, #fff);background:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 5%, #fff)}.step-card span{display:grid;height:26px;width:26px;place-items:center;border-radius:999px;background:#e2e8f0;color:#64748b;font-size:.75rem;font-weight:800}.step-card.active span{background:var(--studio-primary, var(--brand-primary, #7c3aed));color:#fff}.step-card strong,.step-card small{display:block}.step-card strong{margin-top:.7rem;font-size:.8rem;color:#0f172a}.step-card small{margin-top:.2rem;font-size:.7rem;line-height:1.4;color:#64748b}
        .section-manager{overflow:hidden;padding:0}.section-manager-head{display:flex;align-items:flex-end;justify-content:space-between;gap:24px;padding:24px 26px 20px;background:linear-gradient(135deg,#fff 35%,#f7f5ff)}.section-manager-kicker{display:inline-flex;align-items:center;gap:7px;color:var(--studio-primary, var(--brand-primary, #6d28d9));font-size:.68rem;font-weight:900;letter-spacing:.12em;text-transform:uppercase}.section-manager-summary{min-width:170px;border:1px solid color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 25%, #fff);border-radius:14px;background:#fff;padding:11px 14px;box-shadow:0 8px 24px color-mix(in srgb, var(--studio-primary, var(--brand-primary, #6d28d9)) 9%, transparent)}.section-manager-summary span,.section-manager-summary small{display:block}.section-manager-summary span{color:var(--studio-primary, var(--brand-primary, #5b21b6));font-size:.78rem;font-weight:900}.section-manager-summary small{margin-top:2px;color:#94a3b8;font-size:.63rem}.section-drop-note{display:flex;align-items:center;gap:9px;border-block:1px solid color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 14%, #fff);background:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 5%, #fff);padding:11px 26px;color:#6b7280;font-size:.72rem}.section-list{display:grid;gap:10px;padding:18px 20px 22px}.section-row{display:flex;align-items:center;gap:12px;min-height:76px;border:1px solid #e2e8f0;border-radius:15px;background:#fff;padding:10px 12px;box-shadow:0 2px 8px rgb(15 23 42 / .025);transition:border-color .16s,box-shadow .16s,transform .16s,opacity .16s}.section-row:hover{border-color:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 38%, #fff);box-shadow:0 8px 24px color-mix(in srgb, var(--studio-primary, var(--brand-primary, #6d28d9)) 9%, transparent)}.section-row.is-active{border-color:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 38%, #fff);background:linear-gradient(90deg,color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 5%, #fff),#fff 58%)}.section-row:has(.section-check:not(:checked)){opacity:.58}.section-row.is-dragging{z-index:2;opacity:.35;transform:scale(.985)}.section-row.is-drop-before{box-shadow:0 -4px 0 var(--studio-primary, var(--brand-primary, #7c3aed))}.section-row.is-drop-after{box-shadow:0 4px 0 var(--studio-primary, var(--brand-primary, #7c3aed))}.section-drag-handle{display:grid;height:44px;width:34px;flex:0 0 auto;place-items:center;border:1px solid #e2e8f0;border-radius:10px;background:#f8fafc;color:#94a3b8;font-size:1rem;font-weight:900;cursor:grab;letter-spacing:-3px;touch-action:none;user-select:none}.section-drag-handle:active{cursor:grabbing}.section-order-number{display:grid;height:30px;width:30px;flex:0 0 auto;place-items:center;border-radius:9px;background:#f1f5f9;color:#64748b;font-size:.7rem;font-weight:900}.section-row.is-active .section-order-number{background:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 14%, #fff);color:var(--studio-primary, var(--brand-primary, #6d28d9))}.section-row-content{display:flex;min-width:0;flex:1;align-items:flex-start;gap:12px;cursor:pointer}.section-check{margin-top:3px;flex:0 0 auto}.section-row-title{display:flex;align-items:center;flex-wrap:wrap;gap:8px}.section-row-title strong{color:#0f172a;font-size:.84rem}.section-row-title em{border-radius:999px;background:#f1f5f9;padding:3px 7px;color:#64748b;font-size:.58rem;font-style:normal;font-weight:800;text-transform:uppercase}.section-row.is-active .section-row-title em{background:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 14%, #fff);color:var(--studio-primary, var(--brand-primary, #6d28d9))}.section-row-description{display:block;margin-top:4px;color:#64748b;font-size:.7rem}.section-state{min-width:56px;color:#94a3b8;font-size:.62rem;font-weight:900;text-align:center;text-transform:uppercase}.section-row.is-active .section-state{color:#16a34a}.section-actions{display:flex;gap:5px}.section-actions button{display:grid;height:32px;width:32px;place-items:center;border-radius:8px;background:#f1f5f9;color:#475569;font-weight:800}.section-actions button:hover{background:color-mix(in srgb, var(--studio-primary, var(--brand-primary, #7c3aed)) 14%, #fff);color:var(--studio-primary, var(--brand-primary, #6d28d9))}@media(max-width:700px){.section-manager-head{align-items:flex-start;flex-direction:column;padding:20px}.section-manager-summary{width:100%}.section-drop-note{padding-inline:20px}.section-list{padding:14px}.section-order-number,.section-state{display:none}.section-row{gap:8px}.section-actions{flex-direction:column}.section-actions button{height:27px}}
    </style>
    <script>
