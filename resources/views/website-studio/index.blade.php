            ...
            <div id="template-settings" class="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
                <section class="dashboard-card space-y-5">
                    ...
                    <div class="grid gap-4 md:grid-cols-3">
                        ...
                        <label><span class="field-label">Website appearance</span><select name="color_scheme" class="field-input"><option value="dark" @selected(($settings['color_scheme'] ?? 'dark') === 'dark')>Dark mode</option><option value="light" @selected(($settings['color_scheme'] ?? 'dark') === 'light')>Light mode</option></select><span class="mt-1 block text-xs text-slate-400">Sets the public website appearance.</span></label>
                        <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><input type="checkbox" name="theme_switcher_enabled" value="1" @checked(old('theme_switcher_enabled', $settings['theme_switcher_enabled'] ?? true)) class="mt-0.5 size-4 rounded border-slate-300 text-violet-600 focus:ring-violet-500"><span><span class="field-label">Allow visitor theme changes</span><span class="mt-1 block text-xs leading-5 text-slate-500">When disabled, only the appearance selected above is used and visitors cannot switch modes.</span></span></label>
