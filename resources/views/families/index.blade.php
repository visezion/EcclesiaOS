<x-app-layout title="Families & Households Management" :breadcrumbs="$breadcrumbs">
    ...
    <div x-data="{ householdOpen: false, selected: [] }" class="space-y-5">
        ...
        <form method="GET" action="{{ route('families.index') }}" class="dashboard-card grid gap-3 lg:grid-cols-[1fr_180px_auto_auto]">
            ...
            <select name="campus_id" class="h-10 rounded-lg border border-slate-200 px-3 text-sm"><option value="">{{ $terminology['campus_singular'] }}: All</option>@foreach($campuses as $campus)<option value="{{ $campus->opaqueId() }}" @selected($selectedCampusId === $campus->id)>{{ $campus->name }}</option>@endforeach</select>
            ...
            <section class="dashboard-card p-0 xl:col-span-2">
                ...
                <div class="overflow-x-auto">
                    <table class="table-compact min-w-[980px]">
                        <thead><tr><th></th><th>Household ID</th><th>Family Name</th><th>Head of Household</th><th>Members</th><th>Primary {{ $terminology['campus_singular'] }}</th><th>Phone</th><th>Email</th><th>Last Attendance</th><th></th></tr></thead>
                        <tbody>
        ...
        @if($selectedFamily)
            <section class="dashboard-card">
                ...
                <div class="grid gap-4 lg:grid-cols-3">
                    <div class="rounded-lg border border-slate-200 p-4"><h3 class="mb-3 text-sm font-semibold text-slate-950">Household Summary</h3><dl class="space-y-2 text-sm"><div class="flex justify-between"><dt class="text-slate-500">Head</dt><dd class="font-medium">{{ $selectedHead ? $selectedHead->first_name.' '.$selectedHead->last_name : 'Not assigned' }}</dd></div><div class="flex justify-between"><dt class="text-slate-500">Members</dt><dd class="font-medium">{{ $selectedMembers->count() }}</dd></div><div class="flex justify-between"><dt class="text-slate-500">{{ $terminology['campus_singular'] }}</dt><dd class="font-medium">{{ $selectedFamily->campus?->name ?? 'Unassigned' }}</dd></div><div class="flex justify-between"><dt class="text-slate-500">Status</dt><dd class="font-medium text-emerald-600">Good Standing</dd></div></dl></div>
