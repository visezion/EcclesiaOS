...
<div {{ $attributes->merge(['class' => 'searchable-select relative space-y-1 text-sm font-medium text-slate-700']) }}
    :class="open ? 'z-50' : 'z-0'"
    data-menu-layout="{{ $inlineMenu ? 'inline' : 'overlay' }}"
    ...
    <div class="relative" @click.outside="open = false">
        <div class="relative">
            ...
            <input id="{{ $id }}" type="text" x-model="query" @focus="if (! selected) open = true" @input="open = true; selected = ''" @keydown.escape="open = false" placeholder="{{ $placeholder }}" class="w-full rounded-lg border border-slate-200 px-3 py-2.5 pl-9 pr-10 text-sm text-slate-900 focus:border-violet-400 focus:ring-4 focus:ring-violet-100">
        ...
        <div x-cloak x-show="open" class="{{ $inlineMenu ? 'relative' : 'absolute' }} z-[60] mt-2 max-h-72 w-full overflow-y-auto rounded-lg border border-slate-200 bg-white p-2 text-sm shadow-xl">
            @unless($required)
