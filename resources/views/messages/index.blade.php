                        ...
                        <div class="space-y-4 p-5">
                            <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                                <div class="grid gap-3 sm:grid-cols-2">
                                    ...
                                    <select name="conversation_type" x-model="composeConversationType" x-on:change="syncConversationType($el.value)" class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm">
                                        <option value="private">Private</option><option value="group">Group</option><option value="ministry">{{ $terminology['ministry_singular'] }}</option><option value="department">{{ $terminology['ministry_singular'] }}</option><option value="campus">{{ $terminology['campus_singular'] }}</option><option value="role">Role based</option><option value="leadership">Leadership</option><option value="event">Event related</option><option value="task">Task related</option><option value="report">Report related</option><option value="approval">Approval related</option><option value="announcement">Announcement</option>
                                    </select>
                                    </label>
                                    <label class="block text-xs font-bold uppercase tracking-wide text-slate-500">Permission scope
                                        <select name="permission_scope" x-model="composePermissionScope" class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="church">Church</option><option value="campus">{{ $terminology['campus_singular'] }}</option><option value="ministry">{{ $terminology['ministry_singular'] }}</option><option value="leadership">Leadership</option><option value="restricted">Restricted</option></select>
                                    </label>
                                </div>
                            ...
                            <label class="block text-xs font-bold uppercase tracking-wide text-slate-500"><span x-text="'Recipients for ' + composeConversationType.replaceAll('_', ' ')"></span>
                                <select id="message-recipients" name="recipients[]" multiple required class="mt-2 min-h-44 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                                    ...
                                    @if($ministries->isNotEmpty())<optgroup label="{{ $terminology['ministry_plural'] }}" x-show="recipientTypeVisible('ministry')" x-bind:disabled="!recipientTypeVisible('ministry')">@foreach($ministries as $ministry)<option data-recipient-kind="ministry" value="ministry:{{ $ministry->opaqueId() }}">{{ $terminology['ministry_singular'] }}: {{ $ministry->name }}</option>@endforeach</optgroup>@endif
                                    @if($campuses->isNotEmpty())<optgroup label="{{ $terminology['campus_plural'] }}" x-show="recipientTypeVisible('campus')" x-bind:disabled="!recipientTypeVisible('campus')">@foreach($campuses as $campus)<option data-recipient-kind="campus" value="campus:{{ $campus->opaqueId() }}">{{ $terminology['campus_singular'] }}: {{ $campus->name }}</option>@endforeach</optgroup>@endif
