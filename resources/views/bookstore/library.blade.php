<x-app-layout title="Church Library" :breadcrumbs="$breadcrumbs">
    <div x-data="{ productOpen: false, productEditing: null, loanOpen: {{ $errors->any() ? 'true' : 'false' }}, loanEditing: null }" class="space-y-5">
        ...
        <section class="dashboard-card p-0">
            ...
            <form method="GET" action="{{ route('bookstore.library') }}" class="grid gap-3 border-b border-slate-100 p-4 lg:grid-cols-[1fr_150px_150px_170px_auto_auto]">
            ...
            <div class="overflow-x-auto">
                <table class="table-compact min-w-[1120px]">
                    <thead><tr><th>Record</th><th>Book</th><th>Member</th><th>Type</th><th>Due</th><th>Status</th><th>Rental</th><th class="text-right">Actions</th></tr></thead>
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="8" class="px-4 py-12"><x-empty-state icon="library" title="No library records" message="Create the first borrow, rent, or ebook access record." /></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
