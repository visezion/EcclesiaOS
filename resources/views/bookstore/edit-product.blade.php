<x-app-layout title="Edit Product" :breadcrumbs="$breadcrumbs">
    <div class="mx-auto max-w-4xl space-y-5">
        <div class="flex items-center justify-between gap-3">
            <div><h1 class="text-2xl font-semibold text-slate-950">Edit Product</h1><p class="text-sm text-slate-500">{{ $product->name }}</p></div>
        ...
        <section class="dashboard-card">
            <form method="POST" action="{{ route('bookstore.products.update', $product) }}" class="space-y-4">
                @csrf @method('PUT')
                @include('bookstore.partials.product-form', ['product' => $product])
                <div class="flex justify-between gap-2 border-t border-slate-100 pt-4">
                    <button form="delete-product" class="rounded-lg border border-rose-200 px-4 py-2.5 text-sm font-semibold text-rose-700">Archive</button>
            ...
            <form id="delete-product" method="POST" action="{{ route('bookstore.products.destroy', $product) }}" class="hidden">@csrf @method('DELETE')</form>
        </section>
    </div>
</x-app-layout>
