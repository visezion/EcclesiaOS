            ...
            <main class="space-y-4">
                <form id="preferences" x-show="tab === 'preferences'" x-cloak role="tabpanel" aria-labelledby="preferences-tab" method="POST" action="{{ route('account.settings.update') }}" class="dashboard-card">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <label class="text-sm text-slate-600">{{ __('ui.language') }}<select name="language" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm">@foreach(['en' => __('ui.languages.en'), 'fr' => __('ui.languages.fr'), 'es' => __('ui.languages.es')] as $value => $label)<option value="{{ $value }}" @selected(old('language', $preferences['language']) === $value)>{{ $label }}</option>@endforeach</select><span class="mt-1 block text-xs text-slate-400">{{ __('ui.language_help') }}</span></label>
