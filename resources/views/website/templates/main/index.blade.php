@php
    ...
    $heroSlides = collect($settings['hero_slides'] ?? [])->map(fn (array $slide): array => [
    ...
    $youtubeIdFor = static function ($sermon): ?string {
        $id = $sermon?->youtube_video_id;
        if (!$id && preg_match('/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|live\/|embed\/))([^?&\/]+)/i', (string) ($sermon?->video_url ?? ''), $matches)) $id = $matches[1];
        return $id ?: null;
    };
    $thumbnailFor = static function ($sermon) use ($assetUrl, $youtubeIdFor): ?string {
        if (!$sermon) return null;
        $id = $youtubeIdFor($sermon);
        return $assetUrl($sermon->thumbnail_url ?? null) ?: ($id ? 'https://img.youtube.com/vi/'.$id.'/hqdefault.jpg' : null);
    };
    $latestSermon = in_array('sermons', $sections, true) ? $sermons->first() : null;
    $latestThumbnail = $thumbnailFor($latestSermon);
    $sectionPresentation = static fn (array $section): array => ['1200px', 'auto', null, null, 'transparent'];
@endphp
@if (!in_array('experience', $sections, true))<style>.experience{display:none!important}</style>@endif
@if (!in_array('sermons', $sections, true))<style>.feature-strip{display:none!important}</style>@endif
<!doctype html>
<html lang="en">
<head>
...
<body class="theme-{{ $colorScheme }} menu-style-{{ in_array($settings['menu_style'] ?? 'classic', ['classic', 'floating', 'centered', 'pill', 'accent'], true) ? ($settings['menu_style'] ?? 'classic') : 'classic' }}" data-default-theme="{{ $colorScheme }}" data-theme-switcher-enabled="{{ ($settings['theme_switcher_enabled'] ?? true) ? 'true' : 'false' }}" data-theme-key="ecclesia-site-theme-{{ $church->slug }}" style="--primary:{{ $settings['primary_color'] }};--accent:{{ $settings['accent_color'] }};--font:'{{ $settings['font'] ?? 'Manrope' }}',Arial,sans-serif;--hero-media-height:{{ max(300, min(900, (int) ($settings['hero_height'] ?? 560))) }}px">
    ...
    <div class="site-shell">
        ...
        @if (in_array('hero', $sections, true))
            <section class="hero" data-page-section="hero"><div class="container hero-grid"><div class="hero-copy"><p class="eyebrow">{{ $settings['hero_eyebrow'] ?: 'You are welcome here' }}</p><h1>{{ $settings['hero_heading'] ?: $page->title }}</h1><p class="hero-text">{{ $settings['hero_body'] }}</p><div class="button-row"><a class="button" href="{{ $settings['hero_button_url'] ?: '#visit' }}">{{ $settings['hero_button_label'] ?: 'Plan your visit' }} <span>→</span></a><a class="text-link" href="#community">Meet the community <span>↗</span></a></div></div>@include('website.templates.main._hero-slider')</div></section>
        @endif

        @if ($page->slug === 'home' && $latestSermon)<section class="feature-strip"><div class="container feature-grid"><div><p class="eyebrow">Latest message</p><h2>{{ $latestSermon->title }}</h2><p>{{ $latestSermon->summary ?: 'A message to strengthen your faith and help you take your next step.' }}</p><a class="feature-link" href="{{ route('website.public.sermons.show', ['church' => $church->slug, 'sermon' => $latestSermon]) }}">Watch sermon <span>→</span></a></div><div class="feature-image">@if ($latestThumbnail)<img src="{{ $latestThumbnail }}" alt="{{ $latestSermon->title }}" loading="lazy">@else<span>WATCH<br>THE<br>MESSAGE</span>@endif</div></div></section>@endif
        ...
        @if (in_array('sermons', $sections, true) && $sermons->isNotEmpty())<section class="section soft" data-page-section="sermons"><div class="container"><div class="section-heading"><p class="eyebrow">Watch and listen</p><h2>Messages for the journey.</h2></div><div class="card-grid">@foreach ($sermons as $sermon)@php($sermonThumbnail = $thumbnailFor($sermon))<article class="card sermon-card"><a class="sermon-player sermon-player-link" href="{{ route('website.public.sermons.show', ['church' => $church->slug, 'sermon' => $sermon]) }}" aria-label="Open sermon">@if ($sermonThumbnail)<img src="{{ $sermonThumbnail }}" alt="{{ $sermon->title }}" loading="lazy">@else<span class="sermon-card-placeholder">View sermon</span>@endif</a><h3>{{ $sermon->title }}</h3><p>{{ $sermon->speaker ?: 'Teaching team' }} · {{ $sermon->preached_at?->format('M d, Y') ?: 'Recent message' }}</p></article>@endforeach</div></div></section>@endif
        ...
        @foreach ($customNestedSections as $customSection)
            @php([$sectionWidth, $sectionHeight, $sectionBackgroundImage, $sectionBackgroundVideo, $sectionBackgroundColor] = $sectionPresentation($customSection))<section class="section reusable-section reusable-columns has-section-presentation {{ $sectionBackgroundVideo ? 'has-section-video' : '' }}" data-page-section="{{ $customSection['id'] }}" style="--section-content-width:{{ $sectionWidth }};--section-min-height:{{ $sectionHeight }};--section-background-color:{{ $sectionBackgroundColor }};{{ $sectionBackgroundImage ? 'background-image:url('.$sectionBackgroundImage.');' : '' }}">
                @if ($sectionBackgroundVideo)<video class="reusable-section-background" autoplay muted loop playsinline preload="metadata"><source src="{{ $sectionBackgroundVideo }}"></video>@endif
                <div class="container">
...
<script>
const publicSermonUrls = {{ Js::from($sermons->mapWithKeys(fn ($sermon) => [$sermon->title => route('website.public.sermons.show', ['church' => $church->slug, 'sermon' => $sermon])])->all()) }};
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.sermon-card').forEach(function (card) {
        const title = card.querySelector('h3')?.textContent.trim();
        const sermonUrl = publicSermonUrls[title];
        if (sermonUrl) card.addEventListener('click', function () { window.location.href = sermonUrl; });
    });
});
</script>
</body>
