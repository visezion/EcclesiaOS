    ...
    <div class="space-y-5">
        ...
        <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <aside class="space-y-5">
                ...
                @if($canDisburse)
                    <section class="dashboard-card border-emerald-200">
                        <h2 class="flex items-center gap-2 font-black text-slate-950"><i data-lucide="badge-dollar-sign" class="size-4 text-emerald-600"></i>Record disbursement</h2>
                        <p class="mt-1 text-xs text-slate-500">Choose the funding account, record payment, and post the approved amount as an expense.</p>
                        @if($funds->isEmpty())
                            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800"><span class="font-bold">No active funding account is available.</span> Create or activate a fund in Giving &amp; Finance before disbursing.</div>
                        @else
                            <form method="POST" action="{{ route('financial-assistance.disburse', $assistance) }}" class="mt-4 space-y-3">@csrf
                                <label><span class="text-xs font-bold text-slate-700">Funding account / fund *</span><select name="fund_id" required class="mt-1 w-full rounded-lg border-slate-200 text-sm"><option value="">Select the account to charge</option>@foreach($funds as $fund)<option value="{{ $fund->id }}" @selected((string) old('fund_id') === (string) $fund->id)>{{ $fund->name }}{{ $fund->code ? ' ('.$fund->code.')' : '' }}</option>@endforeach</select><span class="mt-1 block text-[10px] text-slate-400">The expense will be assigned to this fund in the finance ledger.</span></label>
                                <label><span class="text-xs font-bold text-slate-700">Expense amount</span><div class="mt-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-black text-slate-800">{{ $assistance->currency }} {{ number_format((float) ($assistance->approved_amount ?? $assistance->amount), 2) }}</div></label>
                                <label><span class="text-xs font-bold text-slate-700">Payment reference *</span><input name="disbursement_reference" required maxlength="120" class="mt-1 w-full rounded-lg border-slate-200 text-sm"></label>
                                <label><span class="text-xs font-bold text-slate-700">Payment date and time *</span><input name="disbursed_at" type="datetime-local" required value="{{ old('disbursed_at', now()->format('Y-m-d\TH:i')) }}" class="mt-1 w-full rounded-lg border-slate-200 text-sm"></label>
                                <label><span class="text-xs font-bold text-slate-700">Notes</span><textarea name="disbursement_notes" rows="3" maxlength="5000" class="mt-1 w-full rounded-lg border-slate-200 text-sm">{{ old('disbursement_notes') }}</textarea></label>
                                <div class="rounded-lg border border-blue-100 bg-blue-50 p-3 text-[10px] leading-4 text-blue-700"><span class="font-bold">Finance ledger entry:</span> Posted expense · Benevolence · {{ $assistance->targetCampus?->name }}</div>
                                <button class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 text-xs font-bold text-white hover:bg-violet-700"><i data-lucide="receipt-text" class="size-4"></i>Disburse and post expense</button>
                            </form>
                        @endif
                    </section>
                @endif
                ...
                @if($assistance->status === 'disbursed')
                    <section class="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><div class="flex gap-3"><i data-lucide="badge-check" class="size-5 text-emerald-600"></i><div class="min-w-0"><div class="text-xs font-black text-emerald-900">Disbursement complete</div><p class="mt-1 text-[10px] leading-4 text-emerald-800">Reference: {{ $assistance->disbursement_reference }}<br>Fund: {{ $assistance->fund?->name ?? 'Not available' }}<br>{{ $assistance->disbursed_at?->format('M d, Y g:i A') }}</p>@if($assistance->financeTransaction)<a href="{{ route('finance.index', ['finance_q' => $assistance->financeTransaction->reference]) }}" class="mt-2 inline-flex items-center gap-1 text-[10px] font-bold text-emerald-800 underline">View ledger expense <i data-lucide="arrow-right" class="size-3"></i></a>@endif</div></div></section>
                @endif
