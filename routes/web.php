...
use App\Http\Controllers\ChurchWebsiteController;
...
Route::middleware(['auth', 'module.enabled'])->group(function (): void {
    ...
    Route::get('support/live/updates', [SupportResourceController::class, 'liveUpdates'])->name('support.live.updates');
    ...
    Route::get('members/imports', [MemberImportController::class, 'index'])->name('member-imports.index');
    Route::get('members/import-connections', [MemberImportConnectionController::class, 'index'])->name('member-import-connections.index');
    Route::post('members/import-connections', [MemberImportConnectionController::class, 'store'])->name('member-import-connections.store');
    ...
    Route::post('members/imports/{memberImport}/rollback', [MemberImportController::class, 'rollback'])->name('member-imports.rollback');
    ...
    Route::delete('website-studio/pages/{page}', [ChurchWebsiteController::class, 'destroyPage'])->name('website-studio.pages.destroy');
    ...
    foreach (collect(config('navigation'))->flatMap(fn (array $item): array => $item['children'] ?? [$item]) as $item) {
        ...
        // Route::has() can miss routes while this file is still being built,
        // which lets the navigation fallback create duplicate named routes.
        // Check the live route collection instead so route:cache remains safe.
        if ($routeName === '' || Route::getRoutes()->getByName($routeName) !== null) {
            continue;
        }

        Route::get(str_replace('.index', '', $routeName), ModuleController::class)->name($routeName);
    }
