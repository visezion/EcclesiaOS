...
- Fixed production framework optimization failing during route caching with `Unable to prepare route [website-studio.sections] for serialization`.
- Fixed duplicate Website Studio routes being generated by the navigation fallback route loop.
- Fixed duplicate `website-studio.media` and `website-studio.sections.create` fallback routes that could also prevent route serialization.
- Fixed the route existence check used while registering navigation fallback routes by checking the live route collection and explicitly protecting all implemented Website Studio child routes.
- Fixed the Docker deployment smoke-test service verification step, which previously combined two Docker commands into one malformed YAML `run` scalar.

### Improved

- Deployment optimization now completes successfully before containers are promoted.
- Route caching is now covered by the release validation process instead of relying only on an uncached route list.
- Docker smoke tests now print service status reliably after startup and continue to preserve diagnostic output when the first status command returns a non-zero result.

### Verification

- `php artisan route:cache` passed after reproducing the original deployment failure.
- `php artisan optimize` passed with configuration, events, routes, and views cached.
- Module route regression suite passed: 16 tests and 550 assertions.
- Composer validation passed.
- Vite production build passed.
- Existing full application verification from v1.0.36 passed: 263 tests and 3,040 assertions.
